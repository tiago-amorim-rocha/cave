...
function createLeg(
  ...
  // NEGATED to flip spider right-side-up (feet should be below body in Y-down coords)
  // Original Unity angles were upside-down for our coordinate system
  ...
  const angle1 = isLeft ? -130 : -50; // Hip absolute angle (negated)
  const angle2 = isLeft ? -100 : 100; // Knee relative to hip (negated)
  const angle3 = isLeft ? -40 : 40; // Ankle relative to knee (negated)
