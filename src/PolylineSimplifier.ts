...
/**
 * Snap vertices to ISO surface using gradient descent
 * Corrects drift from Chaikin smoothing by moving vertices onto the true iso-surface
 *
 * @param polylines - Polylines to snap (e.g., after Chaikin smoothing)
 * @param densityField - Density field to sample
 * @param isoValue - Target ISO value (typically 128)
 * @param gridPitch - Grid pitch (h) for gradient estimation
 * @param maxDisplacement - Maximum allowed displacement (default: 0.5 * gridPitch)
 * @returns Snapped polylines with vertices on ISO surface
 */
export function snapToISOSurface(
  ...
  const maxDist = maxDisplacement * gridPitch;
  let totalVertices = 0;
  let totalDisplacement = 0;
  let maxDist_actual = 0;

  const snapped = polylines.map(polyline => {
    return polyline.map(p => {
      // Sample density at current position using bilinear interpolation
      const F = sampleDensityBilinear(p.x, p.y, densityField, gridPitch) - isoValue;

      // If already very close to ISO, skip
      if (Math.abs(F) < 0.01 * 255) {
        return { x: p.x, y: p.y };
      }

      // Estimate gradient using central differences
      const delta = 0.25 * gridPitch;
      const gx = (sampleDensityBilinear(p.x + delta, p.y, densityField, gridPitch) -
                  sampleDensityBilinear(p.x - delta, p.y, densityField, gridPitch)) / (2 * delta);
      const gy = (sampleDensityBilinear(p.x, p.y + delta, densityField, gridPitch) -
                  sampleDensityBilinear(p.x, p.y - delta, densityField, gridPitch)) / (2 * delta);

      const gradMagSq = gx * gx + gy * gy;

      // If gradient is too small, skip (flat region)
      if (gradMagSq < 1e-12) {
        return { x: p.x, y: p.y };
      }

      // Newton step: move perpendicular to gradient to reach F=0
      const t = F / gradMagSq;
      let dx = t * gx;
      let dy = t * gy;

      // Clamp displacement to maxDist for stability
      ...
      maxDist_actual = Math.max(maxDist_actual, dist);

      return {
        x: p.x - dx, // Subtract because gradient points uphill
        y: p.y - dy
      };
    });
  });
  ...
  // Log statistics
  if (totalVertices > 0) {
    const avgDisplacement = totalDisplacement / totalVertices;
    console.log(`[ISO-Snap Post] ${totalVertices} vertices snapped, avg: ${(avgDisplacement * 1000).toFixed(2)}mm, max: ${(maxDist_actual * 1000).toFixed(2)}mm`);
  }
...
function sampleDensityBilinear(worldX: number, worldY: number, field: any, gridPitch: number): number {
  ...
  const v00 = field.get(gx0, gy0);
  const v10 = field.get(gx1, gy0);
  const v01 = field.get(gx0, gy1);
  const v11 = field.get(gx1, gy1);
